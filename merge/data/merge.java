...
ArrayList SCollection = new ArrayList();
ArrayList GCollection = new ArrayList();
PVector[] roughdots = new PVector[2];
float[] distance = new float[2];
float[] callibrate = new float[2];
...
//BG START  
  ...
  for (int i = 0; i < 61; i++) {
    for (int j = 0; j<209; j++){
      ...
      println(c);
      if (c != 0){
        PVector org = new PVector(i+700,j+400);
        float radius = random(5,15);
        PVector loc = new PVector(org.x+radius, org.y);
        float offSet = random(TWO_PI);
        float dir = 0.1;
  ...
  for (int i = 0; i <119; i++) {
    for (int j = 0; j<200; j++){
      ...
      println(c);
      if (c != 0){
        PVector org = new PVector(i+1100,j+500);
        float radius = random(5,15);
        PVector loc = new PVector(org.x+radius, org.y);
        float offSet = random(TWO_PI);
        float dir = 0.1;
...
void testrun (){
	//Connect S collection
  for (int i=0; i<SCollection.size (); i++) {
    Ball mb = (Ball) SCollection.get(i);
    mb.connection[i] = false;
    mb.run();
    for (int j=i; j<SCollection.size(); j++) {
      Ball other = (Ball) SCollection.get(j);
  ...
  //Connect g collection
  
  for (int i=0; i<GCollection.size (); i++) {
    Ball mb = (Ball) GCollection.get(i);
    mb.connection[i] = false;
    mb.run();
    for (int j=i; j<GCollection.size(); j++) {
      Ball other = (Ball) GCollection.get(j);
      //float distance = loc.dist(other.loc);
      float distance = abs(mb.loc.x - other.loc.x) + abs(mb.loc.y - other.loc.y);
 ...
 for (int j = 0; j < GCollection.size(); ++j) {
   Ball mb = (Ball) GCollection.get(j);
